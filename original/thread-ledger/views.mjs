...
function ticketPrefix(ticket, codes, turl) {
  if (!ticket) return "";
  const cut = ticket.lastIndexOf("#");
  if (cut < 0) return "";
  const [repo, number] = [ticket.slice(0, cut), ticket.slice(cut + 1)];
...
function openRow(thread, nowMsg, codes, sessionUrl, turl) {
...
function closedRow(thread, nowMsg, codes, sessionUrl, turl) {
