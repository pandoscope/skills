...
// A subagent is a hole in the allowlist, not a tool on it: its own
// transcript has no marker, so this driver exits 0 on its every call
// and the read-only policy stops at the boundary. Opus found this on
// the first real opus review (skills#195).
...
function gitWhy(args, run) {
  ...
  // Bare `git stash` is a push and `git tag <name>` creates one: the
  // read forms are the listing ones, and only those pass.
  if (sub === "stash") {
