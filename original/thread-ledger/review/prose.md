...
It found these F findings and H candidates:

```text
{{ candidates }}
```

Each line names a file, a line, a tier and a rule.
Skip the F findings: they are left to CI, not to this review.
Confirm or dismiss each H candidate by reading its line in context.
A confirmed H candidate is a finding.

The check reads one file at a time,
so it never flags an H rule that compares one document with another,
such as `removed-mention` or `drifting-ref`.
Judge those H rules yourself.
Then judge every M rule of the changed files' surfaces.
Judge both across the changed files and the files that they reference.
...
In each finding, `rule` quotes, verbatim, the rules-file sentence that the block violates.
A ticket sentence never counts as a rule.
