...
//                                    criterion and the exact command
//     exit 0                         allowed, or complete
//
// Why two hooks: a Stop hook fires after the turn, so it can refuse
// completion but cannot undo a module that ran or a comment that was
...
export function stopVerdict(run, ctx) {
  ...
  // driver.jsonl holds the denials up to this point — written even
  // when there were none, so an empty file says "measured, nothing
  // denied" and a missing one says "never written". The Stop verdicts
  // that follow stay in the local log: copying them would dirty the
...
if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  ...
  try {
    ...
    // Unparsable stdin: not a review session anyone can act on.
  }
  ...
  if (process.argv.includes("--is-review")) {
    ...
    try {
      ...
      // A crashed driver must not end a review turn quietly, and must
      // not trap it either: block once, release on the guarded fire.
