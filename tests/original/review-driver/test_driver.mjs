...
describe("bash policy", () => {
  ...
  it("denies reading the session's own secrets", () => {
    ...
    // The harness's own persisted tool results are content, not
    // secrets: refusing one cost the first haiku run its diff.
  ...
  it("allows exactly the findings branch, add, commit and push", () => {
    ...
    // A trailing redirect is punctuation by the time the arguments are
    // judged: it must not read as a branch name.
