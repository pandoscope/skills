...
Slot semantics (the JSON `kind` field; compact tags are rendered from it — `matches N`, `my pick`, `cold`, `wildcard`, `alternative`, `free text` — every slot gets at least one). Excluded combinations, enforced by the schema: `cold` × `matches N` (cold means no rule applies, so no slot in the question may carry matches); `wildcard` × `matches N` (a preference-backed branch is not exploratory); `wildcard` × `my pick`/`cold`/`alternative` and `my pick` × `alternative` (kind is single-valued); `cold` only ever accompanies `my pick` (it qualifies the pick, and a second prediction-role slot is rejected); `free text` never combines (renderer-appended slot).
...
Scoring: the session-level `preferences` list is the active set in preference-file order (earlier = higher priority); ranks become weights via normalized 2^-i (lexicographic: every rank outweighs all lower ranks combined, faithful to the set's earlier-rule-wins ordering — the order itself stays human-owned; outcomes propose reorders, never apply them). An option's score = its matched preference weights + `agentScore` (0..1, the agent's own leaning) capped by the top preference weight so agent judgment never outvotes the user's highest-ranked preference. Optional question-level `noneScore` (0..1) is the agent's residual estimate that none of the listed options fit — it scores the free-text slot through the same normalization. Rendered as percent of the question total; hover (page) or the parenthetical (markdown) shows which preference — or the agent's judgment — contributed how much. Session-level `preferenceDocs` maps preference names to their promotion docs; lineage footnotes link them. Fill it from the store's `preferences.json` provenance sidecar (same verbatim-rule-text keys) when the store carries one — never by guessing from the preference line.
